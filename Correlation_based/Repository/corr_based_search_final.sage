...
if VERVOSE == 1:
    print ("===correlation matrix of %s===" % (ALG_NAME))
    if S_BOX_CARDINALITY < 256:
        print (M)
    else:
        print ("corrleation matrix is too lengthy to print")
    print ("========================")
...
if VERVOSE == 1:
    if S_BOX_CARDINALITY < 256:
        print (eigenspace_with_plus_size_value)
        print (eigenspace_with_minus_size_value)
    else:
        print ("eigenspace is too lengthy to print")

if VERVOSE == 1:
    print ("===Nonlinear Invariants for S-Box===")
    print ("Boolean Table input order => 0000, 0001, 0010, ... 1111")
    print ("S-Box(x0, x1, x2, x3):")
    print ("====================================")
...
'''
if _plus_1_eigen_dim != 0:
    the_eigenspace_list = eigenspace_with_plus_size_value.list()
    for walsh in the_eigenspace_list:
        inv_walsh = inv_walsh_trasform(walsh)
        bin_flag = 1
        for value in inv_walsh:
            if value != 0 or value != 1:
                bin_flag = 0
        if bin_flag == 1:
            NUM_NON_INVARIANT+=1
            #Check if the nonlinear invariant has the linear structures.
            if BooleanFunction(inv_walsh).has_linear_structure() == 1:
                NUM_NON_INVARIANT_WITH_LINEAR_STRUCTURE += 1
                #Check if the nonlinear invariant is balanced.
                if BooleanFunction(inv_walsh).is_balanced() == 1:
                    NUM_BALANCED_NON_INVARIANT_WITH_LINEAR_STRUCTURE += 1


if _minus_1_eigen_dim != 0:
    the_eigenspace_list = eigenspace_with_minus_size_value.list()
    for walsh in the_eigenspace_list:
        inv_walsh = inv_walsh_trasform(walsh)
        bin_flag = 1
        for value in inv_walsh:
